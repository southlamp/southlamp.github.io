...
Nodejs + Koa + 腾讯云 cloudBase(云函数 + 云数据库) + 短信验证码 + 短链接 + 钉钉扫码登录
...
vue3.x + element-ui
功能点: 登陆 + 图形验证码 + 权限管理 + 常规增删改查 + 钉钉扫码登录(此处我选择的是内嵌二维码, 样式更灵活)

### 2.踩坑记录

1. 记录一下钉钉扫码成功回调的问题，二维码组件多次渲染，触发了多次回调, 导致调用了多次登录接口。原因在于回调成功是通过监听 message 事件，扫码成功之后收到了多次 message 通知。(我在此处的处理方案是在初始化二维码的时候加入了时间戳, 然后对回调成功的时间戳做判断)
...
13. 钉钉扫码功能实现(先拿到前端传过来的 code，然后通过 code 去钉钉交换 access_token, 再根据 access_token 去交换个人信息)
